...
function mptt_shortcode_template_content_filter() {
	...
	<?php } elseif ( $mptt_shortcode_data[ 'params' ][ 'view' ] == 'tabs' ) { ?>
		<ul class="<?php echo apply_filters( 'mptt_shortcode_navigation_tabs_class', 'mptt-menu mptt-navigation-tabs time-table-ul clear' ) ?>" <?php echo $style ?>>
	...
	<table class="<?php echo ! empty( $table_class ) ? $table_class : ''; ?>" id="#<?php echo is_object( $post ) ? $post->post_name : $post; ?>" style="display:none; <?php echo $font_size; ?>" data-hide_empty_row="<?php echo $hide_empty_rows; ?>">
		...
		<?php if ( isset( $data_grouped_by_row[ 'rows' ] ) && is_array( $data_grouped_by_row[ 'rows' ] ) ) {
			...
			foreach ( $data_grouped_by_row[ 'rows' ] as $row_key => $row ) {
				...
				if(isset($row['cells'][0]['title']) && $row['cells'][0]['title'] ) {

					if(strpos(strtolower($row['cells'][0]['title']),'pm') !== false && !$isCheckedPM ){

						echo View::get_instance()->get_template_html( 'shortcodes/table-header', array( 'header_items' => $data_grouped_by_row[ 'table_header' ], 'params' => $params ) );
						$isCheckedPM = 1;
					}
				}
				...
				<tr class="mptt-shortcode-row-<?php echo $row_key ?>" data-index="<?php echo $row_key ?>">
					...
					foreach ( $cells as $key_event => $cell ) {
						...
						if ( ! $cell[ 'hide' ] ) { ?>
							<td class="mptt-shortcode-event <?php echo mptt_is_grouped_event_class( $cell ) ?> mptt-event-vertical-<?php echo $params[ 'text_align_vertical' ] ?>" data-column-id="<?php echo $cell[ 'column_id' ] ?>" colspan="<?php echo ! isset( $cell[ 'count' ] ) ? '' : $cell[ 'count' ] ?>" data-row_height="<?php echo $row_height; ?>" style="<?php echo 'height:auto'; ?>">
								<?php
